Model file:  mspline_ode_k7.cpp
[PROB]
# Model: `M-spline baseline hazard (7 interior knots), in-solver event detection`
  ...
  (Ramsay 1988), 7 interior knots (`k1`-`k7`) plus boundary knots
  (`bk_lo`, `bk_hi`), evaluated by the Cox-de Boor recursion below --
  10 basis functions / coefficients (`c1`-`c10`) total (interior knots +
  degree + 1).
...
[PARAM] @annotated
  ...
  k1    :  2.5  : M-spline interior knot 1
  k2    :  5  : M-spline interior knot 2
  k3    :  7.5  : M-spline interior knot 3
  k4    :  10  : M-spline interior knot 4
  k5    :  12.5  : M-spline interior knot 5
  k6    :  15  : M-spline interior knot 6
  k7    :  17.5  : M-spline interior knot 7
  ...
  c7    :  1   : M-spline coefficient 7 (must be >= 0)
  c8    :  1   : M-spline coefficient 8 (must be >= 0)
  c9    :  1   : M-spline coefficient 9 (must be >= 0)
  c10    :  1   : M-spline coefficient 10 (must be >= 0)
...
[ODE]
  ...
  double interior[7] = {k1, k2, k3, k4, k5, k6, k7};
  double basis[10];
  mspline_basis(SOLVERTIME, interior, 7, bk_lo, bk_hi, basis);
  double coef[10] = {c1, c2, c3, c4, c5, c6, c7, c8, c9, c10};
  double basehaz = 0.0;
  for (int m = 0; m < 10; m++) basehaz += coef[m] * basis[m];
