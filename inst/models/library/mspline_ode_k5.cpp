Model file:  mspline_ode_k5.cpp
[PROB]
# Model: `M-spline baseline hazard (5 interior knots), in-solver event detection`
  ...
  (Ramsay 1988), 5 interior knots (`k1`-`k5`) plus boundary knots
  (`bk_lo`, `bk_hi`), evaluated by the Cox-de Boor recursion below --
  8 basis functions / coefficients (`c1`-`c8`) total (interior knots +
  degree + 1).
...
[PARAM] @annotated
  ...
  k1    :  3.33  : M-spline interior knot 1
  k2    :  6.67  : M-spline interior knot 2
  k3    :  10  : M-spline interior knot 3
  k4    :  13.33  : M-spline interior knot 4
  k5    :  16.67  : M-spline interior knot 5
...
[ODE]
  ...
  double interior[5] = {k1, k2, k3, k4, k5};
  double basis[8];
  mspline_basis(SOLVERTIME, interior, 5, bk_lo, bk_hi, basis);
  double coef[8] = {c1, c2, c3, c4, c5, c6, c7, c8};
  double basehaz = 0.0;
  for (int m = 0; m < 8; m++) basehaz += coef[m] * basis[m];
